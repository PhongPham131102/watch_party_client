...
import { WatchHistoryItem, UpdateWatchProgressDto } from "@/src/types/watch-history.types";
...
export const useWatchHistoryStore = create<WatchHistoryState>((set, get) => ({
    ...
    fetchHistory: async (page = 1, limit = 20) => {
        ...
        try {
            ...
            if (response.success) {
                const newProgressMap: Record<string, { current: number; total: number }> = {};
    ...
    updateProgress: async (dto: UpdateWatchProgressDto) => {
        ...
        set((state) => {
            const newHistory = state.history.map((item) => {
                if (item.movieId === dto.movieId && (!dto.episodeId || item.episodeId === dto.episodeId)) {
                    return {
                        ...item,
                        watchDurationSeconds: dto.watchDurationSeconds,
                        totalDurationSeconds: dto.totalDurationSeconds || item.totalDurationSeconds,
            ...
            const newProgressMap = {
                ...
                [dto.movieId]: {
                    ...
                    total: dto.totalDurationSeconds || state.progressMap[dto.movieId]?.total || 0
                }
            };
    ...
    getProgress: async (movieId: string, episodeId?: string) => {
        const response = await watchHistoryService.getProgressByMovie(movieId, episodeId);
